...
/**
 * The real `TransferCancelled` consumer for Phase 7 (ADR-025 §6) —
 * mirrors {@see SendTransferConfirmedNotification}'s shape. Deliberately
 * never reads or forwards the event's own `reason` field into the
 * email — content must stay symmetric and blame-free per ADR-018's
 * no-fault posture.
 */
final class SendTransferCancelledNotification implements ShouldQueue
{
    ...
    public function handle(TransferCancelled $event): void
    {
        ...
        if ($snapshot === null) {
            throw new NotificationRecipientUnresolved(
                "TransferCancelled for transfer [{$transferId}]: no matching transfer found."
            );
        }
