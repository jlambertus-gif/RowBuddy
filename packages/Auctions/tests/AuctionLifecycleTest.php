...
it('releasing events clears them so they are not dispatched twice', function () {
    $auction = Auction::open('auction-2', 'queue-1', 'seller-1', 'session-2', usd(1000), aFutureClosesAt(), new FrozenClock);
...
it('transitions from open to closing and raises a closing-started event', function () {
    $auction = Auction::open('auction-3', 'queue-1', 'seller-1', 'session-3', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->releaseEvents();
...
it('selects a winning bid from closing and raises a won event', function () {
    $auction = Auction::open('auction-4', 'queue-1', 'seller-1', 'session-4', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);
    $auction->releaseEvents();
...
it('expires from closing without a winning bid and raises an expired event', function () {
    $auction = Auction::open('auction-5', 'queue-1', 'seller-1', 'session-5', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);
    $auction->releaseEvents();
...
it('cannot start closing an auction that is already closing', function () {
    $auction = Auction::open('auction-6', 'queue-1', 'seller-1', 'session-6', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);
...
it('cannot select a winning bid on an auction that is still open', function () {
    $auction = Auction::open('auction-7', 'queue-1', 'seller-1', 'session-7', usd(1000), aFutureClosesAt(), new FrozenClock);

    $auction->selectWinningBid('bid-1', usd(1500), new FrozenClock);
})->throws(IllegalStateTransition::class);

it('cannot select a winning bid twice, protecting the accepted-bid invariant', function () {
    $auction = Auction::open('auction-8', 'queue-1', 'seller-1', 'session-8', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);
    $auction->selectWinningBid('bid-1', usd(1500), new FrozenClock);
...
it('cannot expire an auction that is still open', function () {
    $auction = Auction::open('auction-9', 'queue-1', 'seller-1', 'session-9', usd(1000), aFutureClosesAt(), new FrozenClock);

    $auction->expireWithoutWinningBid(new FrozenClock);
})->throws(IllegalStateTransition::class);

it('cannot expire an auction that already has a winning bid', function () {
    $auction = Auction::open('auction-10', 'queue-1', 'seller-1', 'session-10', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);
    $auction->selectWinningBid('bid-1', usd(1500), new FrozenClock);
...
it('reconstitutes from persistence without raising any events', function () {
    $auction = Auction::fromPersistence(
        ...
        new DateTimeImmutable('2026-09-10 09:30:00'),
        AuctionStatus::Won,
        'bid-1',
        usd(1500),
    );
...
it('has no proximityAtRiskSince right after opening', function () {
    $auction = Auction::open('auction-13', 'queue-1', 'seller-1', 'session-13', usd(1000), aFutureClosesAt(), new FrozenClock);
...
it('flags an open auction as proximity-at-risk and raises the matching event', function () {
    $auction = Auction::open('auction-14', 'queue-1', 'seller-1', 'session-14', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->releaseEvents();
...
it('flags a closing auction as proximity-at-risk too', function () {
    $auction = Auction::open('auction-15', 'queue-1', 'seller-1', 'session-15', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);
...
it('cannot flag proximity at risk twice', function () {
    $auction = Auction::open('auction-16', 'queue-1', 'seller-1', 'session-16', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->flagProximityAtRisk(new FrozenClock);
...
it('cannot flag proximity at risk on a won auction', function () {
    $auction = Auction::open('auction-17', 'queue-1', 'seller-1', 'session-17', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);
    $auction->selectWinningBid('bid-1', usd(1500), new FrozenClock);
...
it('restores proximity and clears proximityAtRiskSince, raising the matching event', function () {
    $auction = Auction::open('auction-18', 'queue-1', 'seller-1', 'session-18', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->flagProximityAtRisk(new FrozenClock);
    $auction->releaseEvents();
...
it('cannot restore proximity when it was never flagged at risk', function () {
    $auction = Auction::open('auction-19', 'queue-1', 'seller-1', 'session-19', usd(1000), aFutureClosesAt(), new FrozenClock);

    $auction->restoreProximity(new FrozenClock);
})->throws(IllegalStateTransition::class);

it('cancels an open auction for proximity loss and raises a cancelled event', function () {
    $auction = Auction::open('auction-20', 'queue-1', 'seller-1', 'session-20', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->releaseEvents();
...
it('cancels a closing auction for proximity loss even if never flagged at risk first', function () {
    $auction = Auction::open('auction-21', 'queue-1', 'seller-1', 'session-21', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);
...
it('cannot cancel for proximity loss an auction that has already been won', function () {
    $auction = Auction::open('auction-22', 'queue-1', 'seller-1', 'session-22', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);
    $auction->selectWinningBid('bid-1', usd(1500), new FrozenClock);
...
it('exposes the closesAt it was opened with', function () {
    $openedAt = new DateTimeImmutable('2026-09-30 10:00:00');
    $closesAt = new DateTimeImmutable('2026-09-30 10:30:00');

    $auction = Auction::open('auction-23', 'queue-1', 'seller-1', 'session-23', usd(1000), $closesAt, new FrozenClock($openedAt));

    expect($auction->closesAt())->toEqual($closesAt);
});

it('rejects a closesAt that is not after openedAt', function () {
    $openedAt = new DateTimeImmutable('2026-09-30 10:00:00');

    Auction::open('auction-24', 'queue-1', 'seller-1', 'session-24', usd(1000), $openedAt->modify('-1 second'), new FrozenClock($openedAt));
})->throws(InvalidClosingDeadline::class);

it('rejects a closesAt exactly equal to openedAt', function () {
    $openedAt = new DateTimeImmutable('2026-09-30 10:00:00');

    Auction::open('auction-25', 'queue-1', 'seller-1', 'session-25', usd(1000), $openedAt, new FrozenClock($openedAt));
})->throws(InvalidClosingDeadline::class);

it('reconstitutes closesAt from persistence', function () {
    $closesAt = new DateTimeImmutable('2026-09-30 10:30:00');
...
it('extends the closing deadline and raises the matching event', function () {
    $auction = Auction::open('auction-27', 'queue-1', 'seller-1', 'session-27', usd(1000), aFutureClosesAt(), new FrozenClock);
    $originalClosesAt = $auction->closesAt();
    $auction->releaseEvents();

    $newClosesAt = $originalClosesAt->modify('+2 minutes');
    $auction->extendClosingDeadline($newClosesAt, new FrozenClock);

    expect($auction->closesAt())->toEqual($newClosesAt)
        ->and($auction->closesAt())->not->toEqual($originalClosesAt);

    $events = $auction->releaseEvents();
    expect($events)->toHaveCount(1)
        ->and($events[0])->toBeInstanceOf(AuctionClosingDeadlineExtended::class);
});

it('cannot extend the closing deadline on a closing auction', function () {
    $auction = Auction::open('auction-28', 'queue-1', 'seller-1', 'session-28', usd(1000), aFutureClosesAt(), new FrozenClock);
    $auction->startClosing(new FrozenClock);

    $auction->extendClosingDeadline($auction->closesAt()->modify('+2 minutes'), new FrozenClock);
})->throws(IllegalStateTransition::class);
